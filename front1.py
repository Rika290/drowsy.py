...
choice=st.sidebar.selectbox('Dashboard',('Home','IP Camera','File'))
if(choice=='Home'):
    ...
    a=st.text_input('Enter the link for accessing WebCam') # since, 0 is for camera, input is taken in number format..thats why, st.number_input and not ss text_input.
    window=st.empty()
    b=st.button('Start')
    if b:
